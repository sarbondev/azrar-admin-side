  ...
  useEffect(() => {
    if (product) {
      setForm({
        ...
        category:
          (product.category as CategoryEntity)?._id ??
          ...
          <div className="grid gap-4">
            {(["uz", "ru"] as const).map((lang) => (
              <div key={lang} className="grid gap-2">
                <Label>
                  {t(`products.dialog.name${lang === "uz" ? "Uz" : "Ru"}`)}
                </Label>
                <Input
                  value={form.translations[lang].title}
                  onChange={(e) => setTrans(lang, "title", e.target.value)}
                  required
                />
                <Label>
                  {t(
                    `products.dialog.description${lang === "uz" ? "Uz" : "Ru"}`,
                  )}
                </Label>
                <Textarea
                  value={form.translations[lang].description}
                  onChange={(e) =>
                    setTrans(lang, "description", e.target.value)
                  }
                  rows={3}
                  required
                />
            ...
            <ColorPicker
              value={form.colors}
              onChange={(colors) => setForm((p) => ({ ...p, colors }))}
            />
            <div className="grid gap-2">
